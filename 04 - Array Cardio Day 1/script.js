...
const peopleBorn = inventors.filter(function(inventor) {
    if(inventor.year >= 1500 && inventor.year < 1600) {
        return true;
    }
});

console.table(peopleBorn);
...
const oldestToYongest = inventors.sort(function(firstPerson, lastPerson) {
...
// const links = Array.from(category.querySelectorAll('a')) // Node list to array;
...
const vehicles = data.reduce(function(obj, item) {
    // if there is no entry for an item, you need to create one
    // else there will be nothing to add to...returns NaN
    if(!obj[item]) {
        obj[item] = 0;
    }
    obj[item]++;
